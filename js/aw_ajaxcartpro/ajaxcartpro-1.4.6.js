...
if(!Prototype.Browser.IE6){
    ...
    __intId2 = setInterval(
        /* This hangs event listener on @@ items from cart*/
        function(){    
            cnt2--;
...
function updateDeleteLinks(){ // FUNCTION UPDATED
	return false;
	/*
            ...
            if (resp.responseText == 'false') {
                ...
                Event.observe($$('#aw_acp_continue').last(), 'click', function() {
                    if(typeof(optionsPrice) != 'undefined') {
                        if(typeof(optionsPrice.productType) != 'undefined') {
                            switch(optionsPrice.productType) {
                                case 'simple':
                                    acp_remove_configurable_block();
