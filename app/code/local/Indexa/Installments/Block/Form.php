...
class Indexa_Installments_Block_Form extends Indexa_Installments_Block_Abstract
{
	...
	{
		$this->setTemplate('indexa_installments/form.phtml');
	}
	...
	private function _applyExtraParams()
	{
		...
		foreach ($iterable as &$installment)
		{
			...
			{
				if (	$form->getInfoData($this->getPaymentInstallmentCode()) == ""
						|| $form->getInfoData($this->getPaymentInstallmentCode()) == $installment->getValue())
				{
					if( $this->getFormType() ){
	...
	public function getInstallments($getFromSession = false)
	{
        ...
        public function getFormType(){
            return Mage::getStoreConfig('indexa/installments/form_type');
        }
}
