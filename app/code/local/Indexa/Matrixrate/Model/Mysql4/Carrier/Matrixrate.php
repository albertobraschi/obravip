...
*/
class Indexa_Matrixrate_Model_Mysql4_Carrier_Matrixrate extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getNewRate(Mage_Shipping_Model_Rate_Request $request,$zipRangeSet=0)
    {
		...
		$matrixRateCarrierModel = Mage::getModel('matrixrate_shipping/carrier_matrixrate');
		if ($matrixRateCarrierModel->getConfigData('zip_onlynumbers'))
		{
			$postcode = preg_replace("/[^0-9]/","", $postcode );
		}
		
    	if ($matrixRateCarrierModel->getConfigData('zip_ignorelast'))
		{
...
//            }
                ...
                if (empty($exceptions)) {
                    ...
                    foreach ($csvLines as $k=>$csvLine) {
                        ...
                        if (count($csvLine) == 10) {
                            ...
                            if ($csvLine[9] == '') {
                                $csvLine[9] = 0;
                            } elseif (!$this->_isPositiveIntNumber($csvLine[9])) {
                                $exceptions[] = Mage::helper('shipping')->__('Invalid %s To "%s" in the Row #%s', $conditionFullName, $csvLine[9], ($k + 1));
                            }

                            $data[] = array('website_id' => $websiteId, 'dest_country_id' => $countryId, 'dest_region_id' => $regionId, 'dest_city' => $city, 'dest_zip' => $zip, 'dest_zip_to' => $zip_to, 'condition_name' => $conditionName, 'condition_from_value' => $csvLine[5], 'condition_to_value' => $csvLine[6], 'price' => $csvLine[7], 'delivery_type' => $csvLine[8], 'estimated_delivery_time' => $csvLine[9]);
                        } 
						else 
						{
                            ...
                            if ($csvLine[7] == '') {
                                $csvLine[7] = 0;
                            } elseif (!$this->_isPositiveIntNumber($csvLine[7])) {
                                $exceptions[] = Mage::helper('shipping')->__('Invalid %s To "%s" in the Row #%s', $conditionFullName, $csvLine[7], ($k + 1));
                            }

                            $data[] = array('website_id' => $websiteId, 'dest_country_id' => $countryId, 'dest_region_id' => $regionId, 'dest_city' => $city, 'dest_zip' => $zip, 'dest_zip_to' => $zip_to, 'condition_name' => $conditionName, 'condition_from_value' => $csvLine[3], 'condition_to_value' => $csvLine[4], 'price' => $csvLine[5], 'delivery_type' => $csvLine[6], 'estimated_delivery_time' => $csvLine[7]);
                        }
    ...
    private function _isPositiveIntNumber($n) {
        return (substr_count($n, '.') == 0 && substr_count($n, ',') == 0 && intval($n) >= 0) ? true : false;
    }
