...
class Indexa_Gwap_ImprimirController extends Mage_Core_Controller_Front_Action{
        ...
        if( $mOrder->hasData() ){
            ...
            if( $mOrder->getPayment()->getMethod() == 'gwap_boleto'){
                ...
                if( $gwapItem->hasData() && $gwapItem->getType() == 'boleto' ){
                    ...
                    if($gwapItem->getStatus() == Indexa_Gwap_Model_Order::STATUS_CREATED){
                        ...
                        try{
                            ...
                            $gwapItem->setStatus(((Mage::getStoreConfig('indexa/fcontrol/active') && Mage::getStoreConfig('indexa/'.$mOrder->getPayment()->getMethod().'/mc_fraud_check') ) ? Indexa_Gwap_Model_Order::STATUS_AUTHORIZED : Indexa_Gwap_Model_Order::STATUS_CAPTUREPAYMENT));
                            $gwapItem->setErrorCode(null);
                            $gwapItem->setErrorMessage(null);
                            $gwapItem->setTries(0);
                            $gwapItem->setAbandoned(0);
                            $gwapItem->setUpdatedAt(date("Y-m-d H:i:s"));
                            $gwapItem->save();
                            ...
                            if( $gwapItem->getStatus() == Indexa_Gwap_Model_Order::STATUS_CAPTUREPAYMENT ){
                            ...
                            $log->add($gwapItem->getOrderId(), '+ Conversao', 'authorize()', Indexa_Gwap_Model_Order::STATUS_ERROR, 'Ocorreu um erro', $e->getMessage());
                            $gwapItem = Mage::getModel('gwap/order')->load( $itemId );
                            $gwapItem->setUpdatedAt(date("Y-m-d H:i:s"));
                            $gwapItem->save();
                    ...
                    }elseif($gwapItem->getStatus() == Indexa_Gwap_Model_Order::STATUS_CAPTUREPAYMENT){
                        //redirectiona para url do boleto
                        $this->_redirectUrl($gwapItem->getInfo());
                        return ;
                    }
