 ...
 */
class Indexa_Gwap_Helper_Data extends Mage_Core_Helper_Abstract
{
    private $_order;
    private $_config;
    private $_auth;
    ...
    public function prepareData( $type ){
        ...
        if( is_numeric( $order->getShippingAddress()->getCity() ) && Mage::getStoreConfig('indexa_nfe/geral/status') ){
            $city = Mage::getModel('indexa_nfe/directory_country_region_city')->load($order->getShippingAddress()->getCity())->getName();
        }else{
        ...
        switch ($type){
            ...
            case 'BRADESCO':
                ...
                $parameters['CRITERION.BRADESCO_cpfsacado']  = str_replace(array('.','-'), array('', ''), $order->getCustomerTaxvat());
            ...
            case 'ITAU':
                ...
                $parameters['CRITERION.BOLETO_Numberenrollment']  = str_pad( str_replace(array('.','-'), array('', ''), $order->getCustomerTaxvat()), 14, "0", STR_PAD_LEFT);
    ...
    public function getBoletoUrl( $orderId ){
        ...
        return $store->getUrl('indexa_gwap/imprimir/boleto', array('id'=>$order->getId(), 'ci'=>$order->getCustomerId()));
    }
}