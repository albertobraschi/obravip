...
Class BusinessDecision_Interaktingslider_Block_Interaktingslider extends Mage_Core_Block_Template {
	...
	public function startJs(){
		...
		$showcounters = $this->getModel()->getShowCounters() ? 'true' : 'false';
		...
		$vs_Js = '
			...
			interaktingslider.setShowCounters('.$showcounters.'); ';
	...
	public function getSkinCss(){
		...
		try{
			...
			$replace_labels = array('[[IMAGES_FOLDER]]', '[[WIDTH]]', '[[HEIGHT]]', '[[SHOWNUMBERS]]');
			$replace_values = array(
									Mage::getBaseUrl('skin').'interaktingslider/'.$this->getModel()->getSkin().'/images', 
									$this->getModel()->getWidth(), 
									$this->getModel()->getHeight(),
									$this->getModel()->getShowNumbers() ? 'block' : 'none'
									);
			
			
			$vs_Skin = str_replace($replace_labels, $replace_values, $vs_Skin);
			/**
