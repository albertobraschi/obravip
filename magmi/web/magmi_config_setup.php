...
<div  style="display: none;" class="grid_12 subtitle"><span>Update Magmi</span>
</div>
</div>
<div class="container_12">
<?php if($zipok){?>
<form style="display: none;"  method="post" enctype="multipart/form-data" action="magmi_upload.php">
...
<div class="container_12" >
<div class="grid_12 subtitle"><span>Rodar Magmi</span>
<?php if(!$conf_ok){?>
...
<form method="POST" id="runmagmi" action="magmi.php?ts=<?php echo time() ?>" <?php if(!$conf_ok){?>style="display:none"<?php }?>>
	...
	<div class="container_12">
		<div class="grid_12 col" id="directrun">	
			<h3>Rodar magmi com o perfil existente</h3>
			<div class="formline">
				<span class="label">Rodar o magmi com o perfil:</span>
				<?php $profilelist=$conf->getProfileList(); ?>
				<select name="profile" id="runprofile">
					<option <?php if(null==$profile){?>selected="selected"<?php }?> value="default">Padrão</option>
			...
			<span>usando o modo:</span>
				<select name="mode" id="mode">
					<option value="update">Somente atualiza os itens existentes, pula os novos</option>
					<option value="create">Cria novos itens &amp; atualiza os já existentes</option>
					<option value="xcreate">Somente cria os novos itens e pula os já existentes</option>

				</select>
			<input type="submit" value="RODAR!" <?php if(!$conf_ok){?>disabled="disabled"<?php }?>></input>
			</div>
		</div>
		</div>
</form>
<div class="container_12">
<div class="grid_12">
<a style="display: none;"  href="magmi_utilities.php">Advanced Utilities</a>
</div>
</div>
<div style="display: none;"  class="container_12" >
...
?>	
<div class="clear"></div>
<form style="display: none;"  method="post" action="magmi_saveconfig.php" id="commonconf_form">
<div style="display: none;"  class="container_12" id="common_config">
